...
#ifdef _WIN32 // Curl loading stuff on windows
...
#define CURL_PROCS\
	CURL_PROC(curl_version_info_data*, curl_version_info, CURLversion type)\
	CURL_PROC(CURLcode, curl_global_init, long flags)\
	CURL_PROC(CURL*, curl_easy_init)\
	CURL_PROC(CURLcode, curl_easy_setopt, CURL *handle, CURLoption option, const void *parameter)\
	CURL_PROC(CURLcode, curl_easy_perform, CURL *easy_handle)\
	CURL_PROC(void, curl_easy_cleanup, CURL *handle)\
	CURL_PROC(void, curl_global_cleanup, void)\

extern "C" {
#define CURL_PROC(ret, name, ...) typedef ret (name##_proc)(__VA_ARGS__); name##_proc *name;
	CURL_PROCS
#undef CURL_PROC
}

void load_curl_procs() {
	//void *sdl_lib = dlopen("libSDL2.dylib", RTLD_LAZY);
	HMODULE curl_mod = LoadLibraryA("curl.dll");
#define CURL_PROC(ret, name, ...) name = (name##_proc*)GetProcAddress(curl_mod, #name);
	CURL_PROCS
#undef CURL_PROC
}
